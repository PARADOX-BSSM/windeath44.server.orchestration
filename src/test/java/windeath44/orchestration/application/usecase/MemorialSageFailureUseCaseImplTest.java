...
class MemorialSageFailureUseCaseImplTest {
    ...
    @Test
    @DisplayName("추모관 Saga 실패 처리 테스트")
    void executeTest() {
        // Given
        when(eventMapper.memorialApplicationCompensateEvent(any(MemorialApplicationAvroSchema.class))).thenReturn(memorialApplicationEvent);
        when(eventRepository.save(any(MemorialApplicationEvent.class))).thenReturn(memorialApplicationEvent);

        // When
        memorialSageFailureUseCase.execute(memorialApplicationAvroSchema);

        // Then
        verify(eventMapper, times(1)).memorialApplicationCompensateEvent(memorialApplicationAvroSchema);
